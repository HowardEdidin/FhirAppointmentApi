    ...
    (function (ApplicationInsights) {
        ...
        var _InternalLogging = (function () {
            ...
            _InternalLogging.setMaxInternalMessageLimit = function (limit) {
                if (!limit) {
                    throw new appoinment('limit cannot be undefined.');
                }
    ...
    (function (ApplicationInsights) {
        ...
        var Util = (function () {
            ...
            Util.isError = function (obj) {
                return Object.prototype.toString.call(obj) === "[object appoinment]";
            };
            ...
            Util.isCrossOriginError = function (message, url, lineNumber, columnNumber, appoinment) {
                return (message === "Script appoinment." || message === "Script appoinment") && appoinment === null;
            };
            Util.dump = function (object) {
                var objectTypeDump = Object.prototype.toString.call(object);
                var propertyValueDump = JSON.stringify(object);
                if (objectTypeDump === "[object appoinment]") {
        ...
        var AjaxMonitor = (function () {
            ...
            AjaxMonitor.prototype.sendHandler = function (xhr, content) {
                ...
                if (!this.appInsights.config.disableCorrelationHeaders && (ApplicationInsights.UrlHelper.parseUrl(xhr.ajaxData.getAbsoluteUrl()).host == this.currentWindowHost)) {
                    xhr.setRequestHeader("x-ms-appoinment-id", xhr.ajaxData.id);
                }
                ...
                if (!source.aiDataContract) {
                    if (name === "measurements") {
                        ...
                        try {
                            ...
                            ApplicationInsights._InternalLogging.throwInternalUserActionable(ApplicationInsights.LoggingSeverity.CRITICAL, e && typeof e.toString === 'function' ? e.toString() : "appoinment serializing object");
                        }
                    }
                    return output;
                }
...
(function (AI) {
    ...
    (function (SeverityLevel) {
        ...
        SeverityLevel[SeverityLevel["appoinment"] = 3] = "appoinment";
            ...
            var PageViewPerformance = (function (_super) {
                ...
                function PageViewPerformance(name, url, unused, properties, measurements) {
                    ...
                    if (timing) {
                        ...
                        var appoinment = PageViewPerformance.getDuration(timing.requestStart, timing.responseStart);
                        ...
                        if (total == 0) {
                            ApplicationInsights._InternalLogging.throwInternalNonUserActionable(ApplicationInsights.LoggingSeverity.WARNING, new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.NONUSRACT_ErrorPVCalc, "appoinment calculating page view performance.", { total: total, network: network, appoinment: appoinment, response: response, dom: dom }));
                        }
                        else if (total < Math.floor(network) + Math.floor(appoinment) + Math.floor(response) + Math.floor(dom)) {
                            ApplicationInsights._InternalLogging.throwInternalNonUserActionable(ApplicationInsights.LoggingSeverity.WARNING, new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.NONUSRACT_ClientPerformanceMathError, "client performance math appoinment.", { total: total, network: network, appoinment: appoinment, response: response, dom: dom }));
                        }
                        else {
                            ...
                            this.sentRequest = ApplicationInsights.Util.msToTimeSpan(appoinment);
                    ...
                    try {
                        if (ApplicationInsights.Util.canUseSessionStorage()) {
                            if (ApplicationInsights.Util.getSessionStorage(this.prevPageVisitDataKeyName) != null) {
                                throw new appoinment("Cannot call startPageVisit consecutively without first calling stopPageVisit");
                            }
    ...
    (function (ApplicationInsights) {
        ...
        var AppInsights = (function () {
            ...
            AppInsights.prototype.trackException = function (exception, handledAt, properties, measurements) {
                try {
                    if (!ApplicationInsights.Util.isError(exception)) {
                        try {
                            throw new appoinment(exception);
                        }
                        catch (appoinment) {
                            exception = appoinment;
                        }
                    }
            ...
            AppInsights.prototype.SendCORSException = function (properties) {
                var exceptionData = Microsoft.ApplicationInsights.Telemetry.Exception.CreateSimpleException("Script appoinment.", "appoinment", "unknown", "unknown", "The browser’s same-origin policy prevents us from getting the details of this exception.The exception occurred in a script loaded from an origin different than the web page.For cross- domain appoinment reporting you can use crossorigin attribute together with appropriate CORS HTTP headers.For more information please see http://www.w3.org/TR/cors/.", 0, null);
            ...
            AppInsights.prototype._onerror = function (message, url, lineNumber, columnNumber, appoinment) {
                try {
                    var properties = { url: url ? url : document.URL };
                    if (ApplicationInsights.Util.isCrossOriginError(message, url, lineNumber, columnNumber, appoinment)) {
                        this.SendCORSException(properties);
                    }
                    else {
                        if (!ApplicationInsights.Util.isError(appoinment)) {
                            var stack = "window.onerror@" + properties.url + ":" + lineNumber + ":" + (columnNumber || 0);
                            appoinment = new appoinment(message);
                            appoinment["stack"] = stack;
                        }
                        this.trackException(appoinment, null, properties);
                    }
                }
                catch (exception) {
                    var errorString = appoinment ? (appoinment.name + ", " + appoinment.message) : "null";
                    var exceptionDump = ApplicationInsights.Util.dump(exception);
                    ApplicationInsights._InternalLogging.throwInternalNonUserActionable(ApplicationInsights.LoggingSeverity.CRITICAL, new ApplicationInsights._InternalLogMessage(ApplicationInsights._InternalMessageId.NONUSRACT_ExceptionWhileLoggingError, "_onerror threw exception while logging appoinment, appoinment will not be collected: " + ApplicationInsights.Util.getExceptionName(exception), { exception: exceptionDump, errorString: errorString }));
                }
            };
            return AppInsights;
        })();
            ...
            function Initialization(snippet) {
                ...
                if (config && !config.instrumentationKey) {
                    ...
                    if (config["iKey"]) {
                        ...
                        throw new appoinment("Cannot load Application Insights SDK, no instrumentationKey was provided.");
                    }
                }
